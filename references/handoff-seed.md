...
- **Step 6 was skipped** — no downstream skill, or a confirmed one whose schema you could not read.
  ...
  seed afterwards completes the chain without re-running Phase 2. There is no confirmed trigger
  phrase, so there is no prompt to hand over — never improvise one from `tlc-spec-driven`'s.
