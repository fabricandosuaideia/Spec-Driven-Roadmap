...
11. **Interview and Brownfield modes need explicit confirmation.** Enter them only when a check for
    ...
    is never sufficient.
12. **Reuse existing project knowledge before generating any.** Check for the downstream skill's own
...
- `docs/ROADMAP-INDEX.md` — Phase 1, multi-section mode only. Carries the `## Status` block.
- `docs/ROADMAP.md` / `docs/ROADMAP-<slug>.md` — Phase 2 feature lists and coverage tables. In
...
- **Complements its project-init and codebase-mapping triggers.** If the confirmed downstream skill
  has its own such step, Phase 0 reuses that output instead of running 0b or 0c (rule 12). Note these
